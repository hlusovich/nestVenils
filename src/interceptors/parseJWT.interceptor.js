...
let ParseJWTInterceptor = class ParseJWTInterceptor {
    ...
    intercept(context, next) {
        ...
        if (authToken) {
            const splitedAuthorization = authToken.split(' ')[1];
            const secret = this.configService.get('SECRET_KEY');
            if (secret) {
                try {
                    const decoded = jsonwebtoken.verify(splitedAuthorization, secret);
...
//# sourceMappingURL=parseJWT.interceptor.js.map