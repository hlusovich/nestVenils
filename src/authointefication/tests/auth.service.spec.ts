...
describe('AuthService', () => {
  ...
  it('AuthService should login user', async () => {
    const token: Token = await service.login(
      loginData as unknown as ProfileDocument
    );
  ...
  it("AuthService should register user wit sso if he doesn't exist", async () => {
    ...
    expect(token.access_token).toEqual(
      tokenService.sign({
        email: profileCreateDtoWirtSSo.email,
        role: Role.USER,
      })
    );
  ...
  it('AuthService should register user if he  exists', async () => {
    const token: Token = await service.registerWithSSO({
      ...profileCreateDtoWirtSSo,
      email: testProfile1.email,
    });
    expect(token.access_token).toEqual(
      tokenService.sign({ email: testProfile1.email, role: Role.USER })
    );
    expect(mockProfileReporistory.saveProfile).toHaveBeenCalledTimes(2);
  });
  afterAll(async () => {
