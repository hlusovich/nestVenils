...
describe('VinylsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      .useValue(mockProfileReporistory)
      .overrideProvider(VinylsRepository)
      .useValue(mockVinylReporistory)
  ...
  it('VinylService should return vinyl by id', async () => {
    ...
    expect(mockVinylReporistory.getVinylById).toHaveBeenCalledWith(
      testVinyl1.id
    );
  });
  ...
  it('VinylService should add review', async () => {
    ...
    await vinylService.addVinylsReview(
      testVinyl1.id,
      testProfile1.id,
      reviewDto
    );
    expect(mockVinylReporistory.addVinylsReview).toHaveBeenCalledTimes(1);
    const profile: ProfileForRequest | void =
      await profileService.getProfileById(testProfile1.id);
    const vinyl: IVinyl | undefined = await vinylService.getVinylById(
      testVinyl1.id
    );
  ...
  it('VinylService should add bought vinyl', async () => {
    const profile: ProfileForRequest = await vinylService.addBoughtVynil(
      testProfile1.id,
      testVinyl1.id
    );
    expect(mockProfileReporistory.buyVinyl).toHaveBeenCalledWith(
      testProfile1.id,
      { name: testVinyl1.name, id: testVinyl1.id }
    );
    expect(
      profile.boughtedVinyls.find((vinyl) => vinyl.id === testVinyl1.id)?.name
    ).toBe(testVinyl1.name);
  });
  afterAll(async () => {
    await app.close();
  });
