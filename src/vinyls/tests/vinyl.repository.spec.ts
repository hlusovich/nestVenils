...
describe('VinylsRepository', () => {
  ...
  it('should get vinyl by id', async () => {
    const vinylFromBd: VinylDocument | null =
      await vinylsRepository.getVinylById(vinyl.id);
  ...
  it('should add review to vinyl', async () => {
    ...
    const vinylFromBd: VinylDocument | null =
      await vinylsRepository.getVinylById(vinyl.id);
    expect(vinylFromBd?.reviews?.find((r) => r.id === r.id)?.userId).toBe(
      review.userId
    );
  });
  it('should delete vinyl', async () => {
    const vinylFromBdBeforeDeletion: VinylDocument | null =
      await vinylsRepository.getVinylById(vinyl.id);
    expect(vinylFromBdBeforeDeletion?.id).toBe(vinyl.id);
    await vinylsRepository.deleteVinyl(vinyl.id);
    const vinylFromBdAfterDeletion: VinylDocument | null =
      await vinylsRepository.getVinylById(vinyl.id);
