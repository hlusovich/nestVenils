...
describe('ProfileService', () => {
  ...
  it('ProfileService should return profile by id', async () => {
    const profile: ProfileForRequest = await service.getProfileById(
      testProfile1.id
    );
  ...
  it('ProfileService should return profile by email', async () => {
    const profile: ProfileDocument | null = await service.getProfileByEmail(
      testProfile1.email
    );
    expect(profile?.firstName).toBe(testProfile1.firstName);
  });
  it('ProfileService should save profile', async () => {
    ...
    const profile: ProfileForRequest = await service.saveProfile(
      profileForSave
    );
  ...
  it('ProfileService should save profile for sso', async () => {
    ...
    const profile: ProfileForRequest = await service.saveProfileWithSSO(
      profileForSave
    );
    expect(profile?.firstName).toBe(profileForSave.firstName);
    expect(profile).not.toHaveProperty('password');
  ...
  it('ProfileService should update profile', async () => {
    ...
    const profile: ProfileForRequest | void = await service.updateProfile(
      testProfile2.id,
      profileForUpdate,
      testProfile2.email
    );
  ...
  it('ProfileService should add review', async () => {
    ...
    const profile: ProfileForRequest | void = await service.getProfileById(
      review.userId
    );
    ...
    expect(
      profile?.reviews.find((item) => item.id === review.id)?.vinylId
    ).toBe(review.vinylId);
  });
  ...
  it('ProfileService should add boughtedVinyl', async () => {
    ...
    expect(
      profile?.boughtedVinyls.find((item) => item.id === boughtedVinyl.id)?.name
    ).toBe(boughtedVinyl.name);
  });
