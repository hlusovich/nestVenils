...
describe('ProfileRepository', () => {
  ...
  it('should get all profiles', async () => {
    const profilesFromBd: ProfileDocument[] =
      await profileRepository.getAllProfiles();
    expect(profilesFromBd.find((p) => p.id === profile.id)?.firstName).toBe(
      profile.firstName
    );
  }, 10000);
  it('should update profile', async () => {
    ...
    const profileFromBd: ProfileDocument | null =
      await profileRepository.getProfileById(profile.id);
  ...
  it('should get profile by email', async () => {
    const profileFromBd: ProfileDocument | null =
      await profileRepository.getProfileByEmail(profile.email);
    expect(profileFromBd?.id).toBe(profile.id);
  });
  it('should add review to profile', async () => {
    ...
    const profileFromBd: ProfileDocument | null =
      await profileRepository.getProfileById(profile.id);
    expect(profileFromBd?.reviews?.find((r) => r.id === r.id)?.userId).toBe(
      review.userId
    );
  });
  it('should add boughted vinyl to profile', async () => {
    ...
    const profileFromBd: ProfileDocument | null =
      await profileRepository.getProfileById(profile.id);
    expect(
      profileFromBd?.boughtedVinyls?.find((b) => b.id === boughtedVinyl.id)
        ?.name
    ).toBe(boughtedVinyl.name);
  });
  it('should delete profile', async () => {
    const profileFromBdBeforeDeletion: ProfileDocument | null =
      await profileRepository.getProfileById(profile.id);
    expect(profileFromBdBeforeDeletion?.id).toBe(profile.id);
    await profileRepository.deleteProfile(profile.id);
    const profileFromBdAfterDeletion: ProfileDocument | null =
      await profileRepository.getProfileById(profile.id);
