...
export const mockProfileReporistory
    ...
    saveProfile: jest.fn((profileData: IProfileCreateDto): IProfile => {
        const id: string = v4();
        const profile: IProfile = {id, ...profileData, role: Role.USER, reviews: [], boughtedVinyls: []};
    ...
    updateProfile: jest.fn((id: string,
                            newData: IProfileCreateDtoForUpdate): IProfile | undefined => {
    ...
    addReview: jest.fn((userId: string, review: IReview) => {
        const profile: IProfile | undefined = profiles.find(profile => profile.id === userId);
        if (profile) {
